...
// function Dot({ color = 'cyan', pulse = false }) {
...
// function formatIncidentLabel(rawType) {
//   return (rawType || '').replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
// }

// function sanitizeDecision(decision) {
//   return (decision || '').replace(/^\[(FINAL DECISION|DECISION)\]\s*/i, '').trim();
// }
...
// function Stat({ icon, main, sub, color, highlight = false }) {
//   const resolvedColor = COLORS[color] || color;
//   return (
//     <div
//       style={{
...
//         padding: '0 14px',
//         borderRight: '1px solid rgba(255,255,255,0.04)',
//         background: highlight ? `${resolvedColor}08` : 'transparent',
//       }}
//     >
//       <span style={{ fontSize: '14px', lineHeight: 1 }}>{icon}</span>
//       <div>
//         <div style={{ fontSize: '14px', fontWeight: '700', color: resolvedColor, lineHeight: 1 }}>{main}</div>
...
//   const visibleEvents = events.filter(event => event.type !== 'system_check');
...
//           </span>
//           <span style={{ marginLeft: 'auto', fontSize: '11px', color: '#475569' }}>{visibleEvents.length}</span>
//         </>
//       }
//     >
...
//                   }}
//                 >
//                   {formatIncidentLabel(event.type)}
//                 </span>
//                 {isLive && (
//                   <span
//                     style={{
...
//               </div>
//               {event.description && (
//                 <div style={{ fontSize: '11px', color: '#64748b', lineHeight: 1.55 }}>{event.description}</div>
//               )}
//             </div>
//           );
//         })}
...
//           const title = formatIncidentLabel(entry.eventType || entry.type) || `Incident ${String(entry.incidentId || '').slice(-8)}`;
//           const zone = entry.zone ? ZONE_NAMES[entry.zone] || entry.zone : null;
//           const referenceTime = entry.updatedAt || entry.timestamp;
//           const ageSeconds = referenceTime
//             ? Math.max(0, Math.floor((now - new Date(referenceTime).getTime()) / 1000))
//             : 0;
//           const isWorking = tools.length === 0 && ageSeconds < 30;
//           const needsNoDispatchLabel = tools.length === 0 && !isWorking;
//           const decisionSummary = sanitizeDecision(entry.decision);
//           const truncatedSummary =
//             decisionSummary.length > 200 ? `${decisionSummary.slice(0, 200)}...` : decisionSummary;
...
//               style={{
//                 borderRadius: '8px',
//                 border: `1px solid ${isOpen ? 'rgba(0,212,255,0.3)' : 'rgba(255,255,255,0.06)'}`,
...
//                     {title}
//                     {zone && (
//                       <span style={{ fontSize: '11px', color: '#64748b', fontWeight: '400' }}> — {zone}</span>
//                     )}
//                   </div>
//                   <div
//                     style={{
...
//                         }}
//                       >
//                         {dispatched.length} unit{dispatched.length !== 1 ? 's' : ''} sent
//                       </span>
//                     )}
//                     {tools.length > 0 && <span>{tools.length} action{tools.length !== 1 ? 's' : ''}</span>}
//                     {isWorking && (
...
//                       </span>
//                     )}
//                     {needsNoDispatchLabel && <span style={{ color: '#94a3b8' }}>assessed — no dispatch needed</span>}
//                   </div>
//                 </div>
//                 <div style={{ textAlign: 'right', flexShrink: 0 }}>
//                   <div style={{ fontSize: '10px', color: '#334155' }}>
//                     {new Date(entry.timestamp || entry.updatedAt || Date.now()).toLocaleTimeString()}
//                   </div>
//                   <div style={{ fontSize: '9px', color: '#475569', marginTop: '1px' }}>{relativeAgeLabel(ageSeconds)}</div>
...
//                             }}
//                           >
//                             <span style={{ fontSize: '12px', flex: 1, color: ok ? '#94a3b8' : '#f87171' }}>
...
//   const iconMap = {
//     police: { color: '#3b82f6', symbol: '🚔' },
//     fire: { color: '#ff3b5c', symbol: '🚒' },
//     ems: { color: '#00ff88', symbol: '🚑' },
//     traffic: { color: '#ffd700', symbol: '🚦' },
//   };

//   const { color, symbol } = iconMap[type] || { color: '#94a3b8', symbol: '●' };
//   const deployed = status === 'dispatched';

//   return L.divIcon({
//     className: '',
//     html: `<div style="position:relative;font-size:${deployed ? 22 : 18}px;filter:drop-shadow(0 0 ${deployed ? 8 : 3}px ${color});opacity:${deployed ? 1 : 0.7}">${symbol}${deployed ? '<div style="position:absolute;top:-3px;right:-3px;width:7px;height:7px;background:#ff6b35;border-radius:50%;border:1px solid #080d18;animation:pulse 0.8s ease-in-out infinite"></div>' : ''}</div>`,
//     iconSize: [28, 28],
//     iconAnchor: [14, 14],
//   });
// }
...
//           <div>
//             <div style={{ fontSize: '11px', fontWeight: '700', color: '#ff6b35', letterSpacing: '0.06em' }}>
...
//       <MapContainer center={[28.6139, 77.209]} zoom={11} style={{ height: '100%', width: '100%' }} zoomControl={false}>
...
//                   <div style={{ fontWeight: 'bold', color: '#ff3b5c', marginBottom: '4px' }}>
//                     ⚠️ {(incident.type || '').replace(/_/g, ' ').toUpperCase()}
//                   </div>
//                   <div>📍 {zone.name}</div>
//                   <div>Priority: {incident.priority}/10</div>
//                   {incident.description && (
//                     <div style={{ marginTop: '4px', fontSize: '11px', color: '#64748b' }}>{incident.description.slice(0, 80)}</div>
//                   )}
//                 </div>
//               </Popup>
//             </Marker>
...
//           const number = parseFloat(unit.id.replace(/\D/g, '')) || 0;
//           return (
//             <Marker
//               key={unit.id}
//               position={[zone.lat + (number % 5 - 2) * 0.003, zone.lng + (number % 3 - 1) * 0.004]}
//               icon={makeUnitIcon(unit.type, unit.status)}
//             >
//               <Popup>
...
//                   <div>
//                     Status:{' '}
//                     <span style={{ color: unit.status === 'available' ? '#22c55e' : '#f97316' }}>
//                       {unit.status === 'available' ? '✓ Available' : '● On Scene'}
//                     </span>
//                   </div>
    ...
    <div style={{ position:'relative', height:'100%', borderRadius:'10px', overflow:'hidden', border:'1px solid rgba(0,212,255,0.2)' }}>
      ...
      <MapContainer center={[28.6139,77.2090]} zoom={11} style={{height:'100%',width:'100%'}} zoomControl={false}>
        ...
        {Object.values(unitRoutes).map(route => {
          // Convert zone IDs to lat/lng coordinates
          const positions = (route.path || [])
            .map(zoneId => ZONE_META[zoneId])
            .filter(Boolean)
            .map(z => [z.lat, z.lng]);
            ...
            >
              <Popup>
                <div style={{fontFamily:'monospace',fontSize:'12px',minWidth:'180px'}}>
                  ...
                  <div>📍 Route: {route.path.join(' → ')}</div>
                  <div>⏱ ETA: {route.etaMinutes} min</div>
